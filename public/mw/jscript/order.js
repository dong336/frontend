...
function openAddressHistory() {
	openPopup('../order/address_history_pop', 'AddressHistory');
	//location.replace("address_history");
}

//주소록
function openAddressList() {
	openPopup('../order/address_list_pop', 'AddressList');
	//location.replace("address_list");
}
...
function openCoupon(kind) {
	...
	if (useMoney > 0) {
		...
		layerOpen('order_coupon_pop?kind=' + kind);
	}
}
...
function setCoupon(kind, obj) {
	...
	switch (kind) {
		case CONST_COUPON_KIND_GOODS:
			...
			for (var cart in obj) {
				var $row = $('input[name="cart"][value="' + cart + '"]').closest('li');
...
// # 은행선택 확인  : 시작 ######################################################################
function checkPayBank() {
	var f = document.Frm;
	$('#labelPayDepositor').text(
		$(f.payBankIdx).children('option:selected').attr('depositor'),
	);
}
// # 은행선택 확인 : 끝 ######################################################################
...
function checkDealproof() {
	...
	switch (dpIssueKind) {
		...
		case CONST_DEALPROOF_TAX:
			alert(
				'세금계산서에 필요한 내용을 정확히 입력하고,\n반드시 [확인]을 누르셔야 세금계산서가 정상적으로 발급이 됩니다.',
			);
			openPopup('order_tax_pop', 'DealProof', 496, 696);
			//location.replace("order_tax_pop");
			break;
	}
}
...
function setDealProofTax(data) {
	...
	//f.dpPost.value = data.post;
	//f.dpAddr.value = data.addr;
	//f.dpAddrDetail.value = data.addrDetail;
...
function validOrder() {
	...
	var payway = getRadio(f.payway).toNumeric();
	var mobileEscrowPayway = f.mobileEscrowPayway.value.toNumeric(); // 에스크로 결제수단
	...
	if (payway == CONST_PAYWAY_ONLINE) {
		if (isEmpty(f.payBankIdx)) {
			arrMsgPay[arrMsgPay.length] = '은행';
		}
	...
	// 온라인입금 || 결제금액 0 원(적립금, 쿠폰... 사용)
	if (payway == CONST_PAYWAY_ONLINE || settlePrice == 0) {
		// 에스크로 결제
		if (
			(mobileEscrowPayway & CONST_PAYWAY_VIRTUAL) > 0 &&
			settlePrice >= 50000 &&
			getGoodsSize() == 1
		) {
			if (
	...
	// 신용카드, 실시간계좌이체, 가상계좌
		...
		var usableEscrow = false;

		switch (payway) {
			case CONST_PAYWAY_CARD:
				usableEscrow = chkEscrow(mobileEscrowPayway, CONST_PAYWAY_CARD);
				payCard(f, usableEscrow);
				break;
			case CONST_PAYWAY_BANK:
				usableEscrow = chkEscrow(mobileEscrowPayway, CONST_PAYWAY_BANK);
				payBank(f, usableEscrow);
				break;
			case CONST_PAYWAY_VIRTUAL:
				usableEscrow = chkEscrow(mobileEscrowPayway, CONST_PAYWAY_VIRTUAL);
...
function chkEscrow(mobileEscrowPayway, payway) {
	var usableEscrow = false;

	if ((mobileEscrowPayway & payway) > 0) {
		//		if (confirm("에스크로를 이용하실 수 있습니다.\n에스크로를 이용하여 결제를 진행하시겠습니까?")) {
		usableEscrow = true;
		//		}
	}
