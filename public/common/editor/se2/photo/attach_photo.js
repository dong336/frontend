...
function goStartMode() {
	...
	if (sSrc.indexOf('btn_confirm2.png') < 0) {
		welBtnConfirm.attr('src', './images/bt_confirm.gif');
	...
	if (bExceedLimitTotalSize) {
		...
		aFileList.push(
			'	<a onclick="delImage(\'img' +
				nImageInfoCnt +
				'\')"><img class="del_button" src="./images/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>',
		);
		aFileList.push('	</li> ');
...
function html5Upload() {
	...
	sUploadURL = '/common/photo/file_uploader_html5'; //upload URL
...
function callFileUploader() {
	oFileUploader = new jindo.FileUploader(jindo.$('uploadInputBox'), {
		...
		sUrl: '/common/photo/file_uploader', //파일업로드를 처리하는 페이지
		//sCallback : '/SE2/sample/photo_uploader/callback.jsp',		//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
		sCallback: '/common/editor/se2/photo/callback.html', //업로드 이후에 iframe이 redirect될 콜백페이지의 주소
		...
		success: function (oCustomEvent) {
			...
			//multiupload 사용을 위해 추가
			var sizecheck = 'true';
			var imagecount = 1;
			for (var prop in oCustomEvent.htResult) {
				if (prop.indexOf('size') != -1) {
					if (nMaxImageSize < oCustomEvent.htResult[prop]) {
						alert('용량이 초과된 이미지는 첨부하실수 없습니다.');
						sizecheck = 'false';
					}
				}
				//alert(prop);
				if (prop.indexOf('sFileURL') != -1) {
					if (sizecheck == 'false') {
						sizecheck = 'true';
					} else {
						if (imagecount <= nMaxImageCount) {
							//alert(prop + " : "  + oCustomEvent.htResult[prop] + '<br>');
							oCustomEvent.htResult.sFileURL = oCustomEvent.htResult[prop];
							aResult[0] = oCustomEvent.htResult;
							setPhotoToEditor(aResult);
							imagecount++;
						} else {
							alert(nMaxImageCount + '개 이상의 이미지는 첨부하실수 없습니다.');
							break;
						}
					}
				}
