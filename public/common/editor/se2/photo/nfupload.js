...
NFUpload.prototype.Initialize = function (args) {
	...
	this.SetValue(
		'ImgFileBrowse',
		args.nf_img_file_browse,
		'images/btn_file_browse.gif',
	);
	...
	this.SetValue(
		'ImgFileDelete',
		args.nf_img_file_delete,
		'images/btn_file_delete.gif',
	);
...
NFUpload.prototype.SetValue = function (name, value, defValue) {
	if (typeof value == null) this.Variables[name] = defValue;
...
NFUpload.prototype.GetValue = function (name) {
	if (
		typeof this.Variables[name] == null ||
...
NFUpload.prototype.GetFlashTag = function () {
	...
	if (navigator.plugins && navigator.mimeTypes && navigator.plugins.length) {
		...
		flashTag +=
			' Id="' +
			this.GetValue('NFUploadId') +
			'" name="' +
			this.GetValue('NFUploadId') +
			'" quality="high" bgcolor="#869ca7" type="application/x-shockwave-flash"';
		...
		flashTag +=
			'<param name="movie" value="' + this.GetValue('FlashUrl') + '" />';
...
NFUpload.prototype.GetFlashVars = function () {
	...
	vars +=
		'&ColumnHeader1=' + encodeURIComponent(this.GetValue('ColumnHeader1'));
	vars +=
		'&ColumnHeader2=' + encodeURIComponent(this.GetValue('ColumnHeader2'));
	...
	vars +=
		'&DataFieldName=' + encodeURIComponent(this.GetValue('DataFieldName'));
	vars += '&FontFamily=' + encodeURIComponent(this.GetValue('FontFamily'));
	vars += '&FlashUrl=' + encodeURIComponent(this.GetValue('FlashUrl'));
	vars +=
		'&FileOverwrite=' + encodeURIComponent(this.GetValue('FileOverwrite'));
	vars += '&LimitExt=' + encodeURIComponent(this.GetValue('LimitExt'));
	// [2008-10-28] Flash 10 support
	vars +=
		'&ImgFileBrowse=' + encodeURIComponent(this.GetValue('ImgFileBrowse'));
	vars +=
		'&ImgFileBrowseWidth=' +
		encodeURIComponent(this.GetValue('ImgFileBrowseWidth'));
	vars +=
		'&ImgFileBrowseHeight=' +
		encodeURIComponent(this.GetValue('ImgFileBrowseHeight'));
	vars +=
		'&ImgFileDelete=' + encodeURIComponent(this.GetValue('ImgFileDelete'));
	vars +=
		'&ImgFileDeleteWidth=' +
		encodeURIComponent(this.GetValue('ImgFileDeleteWidth'));
	vars +=
		'&ImgFileDeleteHeight=' +
		encodeURIComponent(this.GetValue('ImgFileDeleteHeight'));
	vars +=
		'&TotalSizeText=' + encodeURIComponent(this.GetValue('TotalSizeText'));
	vars +=
		'&TotalSizeFontFamily=' +
		encodeURIComponent(this.GetValue('TotalSizeFontFamily'));
	vars +=
		'&TotalSizeFontSize=' +
		encodeURIComponent(this.GetValue('TotalSizeFontSize'));
