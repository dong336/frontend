...
jindo.RolloverArea = jindo
	.$Class({
		...
		_removeOvered: function (el) {
			this._aOveredElements.splice(
				jindo.$A(this._aOveredElements).indexOf(el),
				1,
			);
		},
		...
		_isInnerElement: function (elParent, elChild) {
			return elParent === elChild
				? true
				: jindo.$Element(elParent).isParentOf(elChild);
		},
		...
		_findRollover: function (el) {
			...
			return jindo.$$.test(el, '.' + sClassName)
				? el
...
jindo.Calendar = jindo
	.$Class({
		...
		_assignHTMLElements: function () {
			...
			if (
				(this.elBtnPrevYear = jindo.$$.getSingle(
					'.' + sClassPrefix + 'btn-prev-year',
					elLayer,
				))
			) {
			...
			if (
				(this.elBtnPrevMonth = jindo.$$.getSingle(
					'.' + sClassPrefix + 'btn-prev-mon',
					elLayer,
				))
			) {
			...
			if (
				(this.elBtnNextMonth = jindo.$$.getSingle(
					'.' + sClassPrefix + 'btn-next-mon',
					elLayer,
				))
			) {
			...
			if (
				(this.elBtnNextYear = jindo.$$.getSingle(
					'.' + sClassPrefix + 'btn-next-year',
					elLayer,
				))
			) {
			...
			this.elTitleYear = jindo.$$.getSingle(
				'.' + sClassPrefix + 'title-year',
				elLayer,
			);
			this.elTitleMonth = jindo.$$.getSingle(
				'.' + sClassPrefix + 'title-month',
				elLayer,
			);
				...
				switch (sType) {
					...
					case 'month':
						...
						sTitle = sTitleFormat
							...
							.replace(/M/g, this.option('aMonthTitle')[nMonth - 1]);
						break;
				}
			} else {
				sTitle = sTitleFormat
		...
		draw: function (nYear, nMonth, bRelative) {
			...
			if (oShownDate && typeof bRelative != 'undefined' && bRelative) {
				var htRelativeDate = this.constructor.getRelativeDate(
					nYear,
					nMonth,
					0,
					oShownDate,
			...
			if (this.fireEvent('beforeDraw', { nYear: nYear, nMonth: nMonth })) {
				...
				var htToday = this.getToday(),
					...
					nPrevMonthLastDate = this.constructor.getLastDate(
						htDatePrevMonth.nYear,
						htDatePrevMonth.nMonth,
					),
					aDate = [],
					bPrevMonth,
					bNextMonth,
				...
				if (nFirstDay > 0) {
					for (
						i = nPrevMonthLastDate - nFirstDay;
...
jindo.Calendar.getRelativeDate = function (nYear, nMonth, nDate, htDate) {
	return this.getDateHashTable(
		new Date(
			htDate.nYear + nYear,
			htDate.nMonth + nMonth - 1,
			htDate.nDate + nDate,
		),
	);
};
...
jindo.LayerManager = jindo
	.$Class({
		...
		getVisible: function () {
			return (this._bIsLayerVisible =
				this._wel.visible() && this._wel.opacity() > 0);
		},
		_check: function (el) {
				...
				if (Math.ceil(self._wel.height()) < self._nLayerHeight) {
					if (self._fireEventBeforeShow()) {
						if (!self.getVisible()) {
							...
							nDuration = Math.ceil(
								nDuration *
									((self._nLayerHeight - self._wel.height()) /
										(self._nLayerHeight - self._nSlideMinHeight)),
							);
						}
						oTransition
							.attach({
							...
							.start(
								...
								},
								{
									height: self
										.option('Transition')
										.fSlideDown.apply(null, [self._nLayerHeight]),
								},
							);
					}
				}
		...
		slideUp: function (nDelay) {
			...
			this._hide(function () {
				...
				if (self.getVisible()) {
					if (self._fireEventBeforeHide()) {
						nDuration = Math.ceil(
							nDuration * (self._wel.height() / self._nLayerHeight),
						);
						oTransition
							.attach({
								end: function (e) {
							...
							.start(
								...
								},
								{
									height: self
										.option('Transition')
										.fSlideUp.apply(null, [self._nSlideMinHeight]),
								},
							);
					}
				}
...
jindo.LayerPosition = jindo
	.$Class({
				...
				if (el && this._welLayer.visible()) {
					if (
						this.fireEvent('beforeAdjust', {
							...
							htAdjustedPosition: this._adjustPosition(
		...
		setPosition: function (htPosition) {
			...
			welLayer
				.css('left', htPosition.nLeft + 'px')
				.css('top', htPosition.nTop + 'px');
			return this;
		},
		...
		_isFullyVisible: function (htPosition) {
			var elLayer = this.getLayer(),
				...
				nScrollTop = oScrollPosition.top,
				nScrollLeft = oScrollPosition.left,
				oClientSize = jindo.$Document().clientSize(),
				nLayerWidth =
					elLayer.offsetWidth +
					(parseInt(welLayer.css('marginLeft')) +
		...
		_mirrorHorizontal: function (htOption) {
			...
			if (this._isPosition(htConvertedOption, 'right')) {
				htConvertedOption.sPosition = htConvertedOption.sPosition.replace(
					/right/,
					'left',
				);
			} else {
		...
		_adjustPosition: function (htPosition) {
			...
			aCandidatePosition.push(
				this.getPosition(this._mirrorHorizontal(htOption)),
			);
			aCandidatePosition.push(this.getPosition(this._mirrorVertical(htOption)));
			aCandidatePosition.push(
				this.getPosition(
					this._mirrorVertical(this._mirrorHorizontal(htOption)),
				),
			);
			for (
				var i = 0, htCandidatePosition;
			...
			if (htTask) {
				if (htTask.sType == 'task') {
					...
					for (var i = 0, nLength = aList.length; i < nLength; i++) {
						...
						for (
							var j = 0, aValue = aList[i].aValue, nJLen = aValue.length;
							j < nJLen;
							j++
						) {
...
jindo.FileUploader = jindo
	.$Class({
		...
		_assignHTMLElements: function () {
			...
			this._elIframe = jindo.$(
				'<iframe name="' +
					sIframeName +
					'" src="' +
					this.option('sCallback') +
					'?blank">',
			);
		...
		upload: function () {
			...
			this.constructor._oCallback[sFunctionName + '_success'] = jindo
				.$Fn(function (oParameter) {
					this.fireEvent('success', { htResult: oParameter });
					delete this
						.constructor._oCallback[oParameter.callback_func + '_success'];
					delete this
						.constructor._oCallback[oParameter.callback_func + '_error'];
					for (var i = 0; i < this._aHiddenInput.length; i++) {
...
jindo.MultipleAjaxRequest = jindo
	.$Class({
		...
		_fireEventBeforeEachRequest: function (nIndex) {
			if (
				this.fireEvent('beforeEachRequest', {
		...
		_fireEventAfterEachResponse: function (nIndex) {
			if (
				this.fireEvent('afterEachResponse', {
					oAjax: this._aAjax[nIndex],
					nIndex: nIndex,
				})
			) {
			...
			if (this._fireEventStart()) {
				...
				jindo.$A(this._aAjaxData).forEach(function (htAjax, i) {
					var fParallelResponseHandler = function (oResponse) {
						...
						var nIndex = self._findAjaxObjectIndexOfResponse(
...
jindo.RolloverClick = jindo
	.$Class({
		...
		_onActivate: function () {
			this._wfClick.attach(
				this._oRolloverArea._elArea,
				this.option('sCheckEvent'),
			);
...
jindo.Pagination = jindo
	.$Class({
			...
			if (this.option('sMoveUnit') == 'page') {
				...
				switch (sRelative) {
					case 'pre_end':
						...
						break;
					case 'pre':
						var nLastPageOfPrePageList =
							(nThisPageList - 1) * this.option('nPagePerPageList');
						nPage = nLastPageOfPrePageList;
						break;
					case 'next':
						var nFirstPageOfNextPageList =
							nThisPageList * this.option('nPagePerPageList') + 1;
		...
		_getPageElement: function (el) {
			for (
				var i = 0, nLength = this._waPage.$value().length;
				i < nLength;
				i++
			) {
		...
		_paginate: function (nPage) {
			...
			if (htOption.bAlignCenter) {
				...
				nLastPageOfThisPageList =
					nFirstPageOfThisPageList + htOption.nPagePerPageList - 1;
				...
				nFirstPageOfThisPageList =
					(nThisPageList - 1) * htOption.nPagePerPageList + 1;
			...
			var el, wel;
			for (
				var i = nFirstPageOfThisPageList;
				...
				if (i == nPage) {
					el = jindo.$(
						jindo
							.$Template(htOption.sCurrentPageTemplate)
							.process({ page: i.toString() }),
					);
				} else {
					el = jindo.$(
						jindo
							.$Template(htOption.sPageTemplate)
							.process({ page: i.toString() }),
					);
	...
	.$Class({
		...
		removeDatePickerSet: function (ht) {
			...
			for (var i = 0, len = this._aDatePickerSet.length; i < len; i++) {
				if (
					this._aDatePickerSet[i].elInput == ht.elInput ||
		...
		getDatePickerSet: function (el) {
			...
			for (var i = 0, len = this._aDatePickerSet.length; i < len; i++) {
				if (
					this._aDatePickerSet[i].elInput == el ||
				...
				draw: function (oCustomEvent) {
					...
					if (self.isSelectable(oShowDatePickerSet, oCustomEvent)) {
						...
						if (jindo.Calendar.isSameDate(oCustomEvent, oShowDatePickerSet)) {
							jindo
								.$Element(oCustomEvent.elDateContainer)
		...
		_setDate: function (htDatePickerSet, htDate) {
			...
			if (typeof htDatePickerSet.elInput != 'undefined') {
				htDatePickerSet.elInput.value = this._getDateFormat(
					htDatePickerSet,
					htDate,
				);
			}
		...
		_getDateFormat: function (htDatePickerSet, htDate) {
			...
			sDateFormat = sDateFormat
				...
				.replace(/mm/g, nMonth)
				.replace(/m/g, nMonth * 1)
				.replace(/M/g, this.getCalendar().option('aMonthTitle')[nMonth - 1])
				.replace(/dd/g, nDate)
				.replace(/d/g, nDate * 1);
			return sDateFormat;
		},
		...
		_onActivate: function () {
			var elCalendarLayer = this.getCalendarLayer();
			this._wfMouseOverOutDate
				.attach(elCalendarLayer, 'mouseover')
		...
		_onDeactivate: function () {
			var elCalendarLayer = this.getCalendarLayer();
			this._wfMouseOverOutDate
				.detach(elCalendarLayer, 'mouseover')
				.detach(elCalendarLayer, 'mouseout');
			this._wfClickDate
				.detach(elCalendarLayer, 'click')
				.detach(elCalendarLayer, 'mouseover')
				.detach(elCalendarLayer, 'mouseout');
				...
				if (htDatePickerSet) {
					...
					if (this.option('bUseLayerPosition')) {
						if (typeof htDatePickerSet.elLayerPosition != 'undefined') {
							this.getLayerPosition()
								.setElement(htDatePickerSet.elLayerPosition)
								.setPosition();
						} else {
							this.getLayerPosition()
								.setElement(htDatePickerSet.elInput)
								.setPosition();
						}
					}
				}
		...
		_getTargetDateElement: function (el) {
			...
			var elDate = jindo.$Element(el).hasClass(sClassPrefix + 'date')
				? el
				: jindo.$$.getSingle('.' + sClassPrefix + 'date', el);
		...
		_getTargetDateContainerElement: function (el) {
			...
			if (elWeek) {
				var elReturn = el;
				while (
					!jindo.$Element(elReturn.parentNode).hasClass(sClassPrefix + 'week')
				) {
		...
		_onMouseOverOutDate: function (we) {
			...
			if (elDateContainer) {
				...
				if (
					...
					this.isSelectable(this._htSelectedDatePickerSet, htDate)
				) {
						...
						if (!this._elSelected) {
							...
							if (this._elSelected) {
								jindo
									...
									.removeClass(sClassPrefix + 'selected');
							}
						}
...
jindo.StarRating = jindo
	.$Class({
		...
		getValueByWidth: function () {
			return (
				(this._welRatingElement.width() / this._nBaseWidth) *
		...
		_onMouseMove: function (we) {
			...
			if (this.option('bSnap')) {
				...
				nWidth =
					(this._round(nValue, this.option('nStep')) * this._nBaseWidth) /
					this.option('nMaxValue');
				nWidth = Math.min(nWidth, this._nBaseWidth);
			}
...
jindo.DragArea = jindo
	.$Class({
		...
		_findDraggableElement: function (el) {
			if (
				el.nodeType === 1 &&
				jindo.$$.test(el, 'input[type=text], textarea, select')
			) {
				return null;
			}
			var self = this;
			...
			var elReturn = jindo.$$.test(el, sClass)
				? el
		...
		_onMouseMove: function (we) {
			...
			if (htDragInfo.bPrepared) {
				...
				if (!htDragInfo.bForceDrag && nThreshold) {
					...
					var nDistance = Math.sqrt(
						htDiff.nPageX * htDiff.nPageX + htDiff.nPageY * htDiff.nPageY,
					);
			...
			if (this.fireEvent('beforeDrag', htParam)) {
				...
				if (this.option('bFlowOut') === false) {
					...
					if (htParam.nX !== null) {
						...
						htParam.nX = Math.min(
							htParam.nX,
							htRect.nWidth - aSize[0] + nScrollLeft,
						);
					}
					if (htParam.nY !== null) {
						...
						htParam.nY = Math.min(
							htParam.nY,
							htRect.nHeight - aSize[1] + nScrollTop,
						);
					}
				}
				if (htParam.nX !== null) {
...
jindo.NumericStepper = jindo
	.$Class({
		...
		_assignHTMLElements: function () {
			...
			this._elMinusButton = jindo.$$.getSingle(
				'.' + sPrefix + 'minus',
				this._el,
			);
		},
		...
		reset: function () {
			this._elInput.value = this.option('nDefaultValue').toFixed(
				this.option('nDecimalPoint'),
			);
		},
		getValue: function () {
		...
		setValue: function (n) {
			...
			var nMin = this.option('nMin'),
				...
				htParam = { nValue: n, nMin: nMin, nMax: nMax };
			if (n > nMax || n < nMin) {
