...
nhn.husky.SE2M_TableTemplate = [
	{},
	{
		...
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#ffffff', color: '#666666' },
		],
	},
	{
		htTableProperty: { border: '0', cellPadding: '0', cellSpacing: '1' },
		...
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#ffffff', color: '#666666' },
			{ padding: '3px 4px 2px', backgroundColor: '#f3f3f3', color: '#666666' },
		],
	},
	{
	...
	},
	{
		...
		htTableStyle: { backgroundColor: '#c7c7c7' },
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#f8f8f8', color: '#666666' },
			{ padding: '3px 4px 2px', backgroundColor: '#ebebeb', color: '#666666' },
		],
	},
	{
		htTableProperty: { border: '0', cellPadding: '0', cellSpacing: '0' },
	...
	},
	{
		...
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#ffffff', color: '#3d76ab' },
		],
	},
	{
		htTableProperty: { border: '0', cellPadding: '0', cellSpacing: '1' },
		...
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#ffffff', color: '#3d76ab' },
			{ padding: '3px 4px 2px', backgroundColor: '#f6f8fa', color: '#3d76ab' },
		],
	},
	{
		htTableProperty: { border: '0', cellPadding: '0', cellSpacing: '0' },
	...
	},
	{
		...
		htTableStyle: { backgroundColor: '#a6bcd1' },
		aRowStyle: [
			{ padding: '3px 4px 2px', backgroundColor: '#fafbfc', color: '#3d76ab' },
			{ padding: '3px 4px 2px', backgroundColor: '#e6ecf2', color: '#3d76ab' },
		],
	},
	{
		htTableProperty: { border: '0', cellPadding: '0', cellSpacing: '0' },
...
nhn.husky.HuskyCore = jindo.$Class({
	...
	registerBrowserEvent: function (e, f, b, d, a) {
		d = d || [];
		var c = a
			? jindo.$Fn(this.delayedExec, this).bind(b, d, a)
		...
		if (b.match(/^\$(BEFORE|ON|AFTER)_MSG_APP_READY$/)) {
			...
			for (d = 0; d < e; d++) {
				if (!f[d]._husky_bRun) {
					...
					if (
						typeof f[d].$LOCAL_BEFORE_FIRST == 'function' &&
						this._execHandler(f[d], '$LOCAL_BEFORE_FIRST', [b, c]) === false
					) {
						continue;
					}
				}
...
nhn.husky.APP_STATUS = { NOT_READY: 0, WAITING_FOR_PLUGINS_READY: 1, READY: 2 };
...
nhn.W3CDOMRange = jindo.$Class({
	...
	_constructClonedTree: function (b, d) {
		...
		return { oStartContainer: a, oEndContainer: c };
	},
	cloneRange: function () {
	...
	_isCollapsed: function (f, e, d, c) {
		...
		if (f == d && e == c) {
			...
			if (
				...
				g &&
				g.tagName != 'BODY' &&
				(this._getNextNode(g) == a || (g == a && this._isBlankTextNode(g)))
			) {
				b = true;
			}
		}
...
nhn.HuskyRange = jindo
	.$Class({
		...
		_evaluateXPathBookmark: function (h) {
			...
			return { elContainer: e, nOffset: c };
		},
		getXPathBookmark: function () {
			var a = -1;
			var d = { elContainer: this.startContainer, nOffset: this.startOffset };
			...
			var j = { sXPath: h, nTextNodeIdx: a, nOffset: d.nOffset };
			if (this.collapsed) {
				var g = { sXPath: h, nTextNodeIdx: a, nOffset: d.nOffset };
			} else {
				var k = -1;
				var b = { elContainer: this.endContainer, nOffset: this.endOffset };
				...
				var g = { sXPath: f, nTextNodeIdx: k, nOffset: b.nOffset };
			}
			return [j, g];
		},
		...
		_getFixedTextContainer: function (a, b) {
			...
			return { elContainer: a, nOffset: b };
		},
		...
		getStringBookmark: function (b, a) {
			if (a) {
				return this._document.getElementById(
					this.HUSKY_BOOMARK_END_ID_PREFIX + b,
				);
			} else {
				return this._document.getElementById(
					this.HUSKY_BOOMARK_START_ID_PREFIX + b,
				);
			}
		},
		...
		_setTextDecoration: function (a, b) {
			if (jindo.$Agent().navigator().firefox) {
				a.style.textDecoration = a.style.textDecoration
		...
		_checkTextDecoration: function (e) {
			...
			while (oChildNode) {
				if (oChildNode.nodeType === 1) {
					...
					a =
						a || oChildNode.tagName === 'S' || oChildNode.tagName === 'STRIKE';
				}
				if (c && a) {
					return;
				}
		...
		_getLineStartInfo: function (f) {
			...
			return { oNode: h, oLineBreaker: g, bParentBreak: e };
		},
		_getLineEndInfo: function (f) {
			...
			return { oNode: b, oLineBreaker: g, bParentBreak: e };
		},
		getLineInfo: function (h) {
			...
			var a = c.oNode;
			if (b != f || e != a) {
				var g = this._compareEndPoint(
					nhn.DOMFix.parentNode(f),
					this._getPosIdx(f),
					this.endContainer,
					this.endOffset,
				);
				var d = this._compareEndPoint(
					nhn.DOMFix.parentNode(a),
					this._getPosIdx(a) + 1,
					this.startContainer,
					this.startOffset,
				);
...
nhn.BrowserSelectionImpl_IE = function () {
	...
	this.compareEndPoints = function (d, c, b, a) {
		var f, e;
		if (
			d === this.oLastRange.elStartContainer &&
			...
			if (
				d === this.oLastRange.elEndContainer &&
		...
		if (
			b === this.oLastRange.elStartContainer &&
			...
			if (
				b === this.oLastRange.elEndContainer &&
	...
	this._getSelectableNodeAndOffsetForIE = function (e, g) {
		...
		return { oSelectableNodeForIE: a, iOffsetForIE: f, bCollapseToStart: j };
	};
...
nhn.husky.CorePlugin = jindo.$Class({
	...
	$ON_LOAD_FULL_PLUGIN: function (f, b, e, c, d) {
		...
		if (!this.htLazyLoadRequest_plugins[a]) {
			this.htLazyLoadRequest_plugins[a] = { nStatus: 1, sContents: '' };
		}
	...
	_loadFullPlugin: function (f, a, e, c, d, b) {
		jindo.LazyLoading.load(
			nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + '/' + f[b],
			jindo
				.$Fn(function (o, h, n, l, m, k) {
					var j = o[k];
					var g = o[0];
					if (k == o.length - 1) {
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	$ON_EVENT_TOOLBAR_MOUSEDOWN: function (b) {
		...
		while (a) {
			if (
				...
				(a.childNodes.length > 2 || a.parentNode.className.match(/se2_pair/))
			) {
				return;
			}
			a = a.parentNode;
	...
	_setRoundedCornerButton: function () {
		var e, g, d, c, a, f, b, h;
		f = jindo.$$(
			'>ul>li[class*=' + this.sUIClassPrefix + ']:only-child',
			this.elTextTool,
		);
		for (e = 0, g = f.length; e < g; e++) {
			...
			jindo
				.$Element(jindo.$$.getSingle('button>span._buttonRound', d))
				.addClass('tool_bg');
		}
		b = jindo.$$(
			'ul>li[class*=' + this.sUIClassPrefix + ']:first-child',
			this.elTextTool,
		);
		for (e = 0, g = b.length; e < g; e++) {
			...
			c = jindo.$Element(d);
			if (
				c.hasClass('husky_seditor_ui_fontName') ||
				c.hasClass('husky_seditor_ui_text_more') ||
				c.hasClass('husky_seditor_ui_photo_attach') ||
				c.hasClass('husky_seditor_ui_map_attach') ||
		...
		h = jindo.$$(
			'ul>li[class*=' + this.sUIClassPrefix + ']:last-child',
			this.elTextTool,
		);
		for (e = 0, g = h.length; e < g; e++) {
			...
			c = jindo.$Element(d);
			if (
				c.hasClass('husky_seditor_ui_fontSize') ||
				c.hasClass('husky_seditor_ui_text_more') ||
				c.hasClass('husky_seditor_ui_photo_attach') ||
				c.hasClass('husky_seditor_ui_map_attach') ||
				c.hasClass('single_child')
...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	$init: function (g, f, a, e, d) {
		...
		var b = this._getSize(
			[a.nWidth, a.width, this.elEditingAreaContainer.offsetWidth],
			this.nMinWidth,
		);
		var c = this._getSize(
			[a.nHeight, a.height, this.elEditingAreaContainer.offsetHeight],
			this.nMinHeight,
		);
	...
	$ON_RESIZE_EDITING_AREA_BY: function (b, f) {
		...
		if (
			...
			c = this.elEditingAreaContainer.style.width
				? parseInt(this.elEditingAreaContainer.style.width, 10) + d
				: null;
		}
		if (a !== 0) {
			e = this.elEditingAreaContainer.style.height
				? this.iStartingHeight + a
				: null;
		}
		if (!b && !a) {
			return;
	...
	$ON_KEEP_WITHIN_EDITINGAREA: function (b, a) {
		...
		if (c + b.offsetHeight > this.oApp.elEditingAreaContainer.offsetHeight) {
			if (typeof a == 'number') {
				...
				b.style.top =
					this.oApp.elEditingAreaContainer.offsetHeight - b.offsetHeight + 'px';
			}
		}
		var d = parseInt(b.style.left, 10);
		if (d + b.offsetWidth > this.oApp.elEditingAreaContainer.offsetWidth) {
			b.style.left =
				this.oApp.elEditingAreaContainer.offsetWidth - b.offsetWidth + 'px';
		}
	},
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	$ON_MSG_APP_READY: function () {
		...
		if (
			this.isUseVerticalResizer() === false &&
	...
	_assignHTMLElements: function (b, a) {
		...
		this.welNoticeLayer = jindo.$Element(
			jindo.$$.getSingle('DIV.husky_seditor_resize_notice', b),
		);
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
	...
	_setAutoResize: function () {
		...
		this.nTopBottomMargin =
			this.nTopBottomMargin ||
			parseInt(c.css('marginTop'), 10) + parseInt(c.css('marginBottom'), 10);
		this.nBodyMinHeight =
			this.nBodyMinHeight ||
	...
	_setBodyHeight: function () {
		...
		var e = this.getDocument().body,
			b = jindo.$Element(e),
			h =
				parseInt(b.css('marginTop'), 10) + parseInt(b.css('marginBottom'), 10),
	...
	$ON_CHANGE_EDITING_MODE: function (a, b) {
		if (a === this.sMode) {
			...
			if (c.ie && c.nativeVersion > 8) {
				var d = jindo.$$.getSingle('DIV.husky_seditor_editing_area_container')
					.childNodes[0];
	...
	setIR: function (h) {
		...
		if (
			this.iframe.contentWindow.document.body.innerHTML.replace(
				/[\r\n\t\s]*/,
				'',
			) === ''
		) {
...
nhn.husky.SE_PasteHandler = jindo.$Class({
	...
	$ON_MSG_APP_READY: function () {
		if (
			...
			if (
				this.htBrowser.ie &&
				this.htBrowser.nativeVersion <= 10 &&
	...
	_handlePaste: function (c) {
		if (
			...
			d.innerHTML = b;
			var a = jindo.$$.getSingle('style', d, { oneTimeOffCache: true });
			if (a) {
				var e = a.innerHTML;
				e = e.replace(/"/g, "'");
	...
	_saveOriginalContents: function () {
		...
		if (!this._sBM_IE) {
			...
			if (
				this.htBrowser.ie &&
				this.htBrowser.nativeVersion == 11 &&
...
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
	...
	_addBlankText: function (c) {
		...
		for (b = 0, e = g.length; b < e; b++) {
			...
			if (
				f.id.indexOf(c.HUSKY_BOOMARK_START_ID_PREFIX) > -1 ||
			...
			if (
				!d ||
				(d.nodeType == 3 && nhn.husky.SE2M_Utils.isBlankTextNode(d)) ||
				(d.nodeType == 1 &&
					f.childNodes.length == 1 &&
					(d.id.indexOf(c.HUSKY_BOOMARK_START_ID_PREFIX) > -1 ||
						d.id.indexOf(c.HUSKY_BOOMARK_END_ID_PREFIX) > -1))
			) {
	...
	_wrapBlock: function (k, m) {
		...
		if (this.htBrowser.firefox) {
			...
			if (this.htBrowser.ie) {
				...
				b =
					f.tagName === 'U' ||
					nhn.husky.SE2M_Utils.findAncestorByTagName('U', f) !== null;
				a =
					f.tagName === 'S' ||
					f.tagName === 'STRIKE' ||
					(nhn.husky.SE2M_Utils.findAncestorByTagName('S', f) !== null &&
						nhn.husky.SE2M_Utils.findAncestorByTagName('STRIKE', f) !== null);
				if (b || a) {
...
nhn.ColorPicker = jindo
	.$Class({
		...
		$init: function (c, b) {
			...
			this.canvasType = jindo.$(c).filters
				? 'Filter'
			...
			this.option({ huePanel: null, huePanelType: 'horizontal' });
			this.option(b);
		...
		hsv: function (d) {
			...
			this.cursor.css({ left: j + 'px', top: g + 'px' });
			this._hsvColor = d;
		...
		_paintOneWithCanvas: function () {
			var c = { r: 0, g: 0, b: 0 };
			var g = jindo.$Element('<canvas>').css({ width: '100%', height: '100%' });
	...
	.$Class({
		...
		_assignHTMLElements: function (b) {
			...
			if (a) {
				...
				this.URL_COLOR_ADD =
					a.addColorURL || 'http://api.se2.naver.com/1/colortable/TextAdd.nhn';
		...
		_redrawRecentColorElement: function () {
			...
			for (b = 0; b < c; b++) {
				a.push(
					this.sRecentColorTemp.replace(/\{RGB_CODE\}/gi, this.aRecentColor[b]),
				);
			}
		...
		_ajaxAddColor: function () {
			jindo
				.$Ajax(this.URL_COLOR_ADD, { type: 'jsonp', onload: function () {} })
				.request({
					text_key: 'colortable',
		...
		_ajaxUpdateColor: function () {
			jindo
				.$Ajax(this.URL_COLOR_UPDATE, { type: 'jsonp', onload: function () {} })
				.request({
					text_key: 'colortable',
					text_data: this.aRecentColor.join(','),
				});
...
nhn.husky.SE2M_ExecCommand = jindo.$Class({
	...
	$BEFORE_EXECCOMMAND: function (a, j, g, f) {
		...
		if (
			a.match(/^bold|underline|italic|strikethrough|superscript|subscript$/i)
		) {
	...
	_outdentMargin: function (d) {
		...
		if (d.tagName === 'LI') {
			...
			if (
				d.previousSibling &&
				d.previousSibling.tagName &&
				d.previousSibling.tagName.match(/LI|UL|OL/)
			) {
				if (
					d.nextSibling &&
					d.nextSibling.tagName &&
...
nhn.husky.SE2M_FontColor = jindo.$Class({
	...
	$ON_APPLY_FONTCOLOR: function (a) {
		...
		this.oApp.exec('SET_WYSIWYG_STYLE', [{ color: a }]);
		this.oApp.exec('HIDE_ACTIVE_LAYER');
	},
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	addAllFonts: function () {
		...
		if (!!this.aAdditionalFontList && this.aAdditionalFontList.length > 0) {
			for (c = 0, nLen = this.aAdditionalFontList.length; c < nLen; c++) {
				this.addFont(
					this.aAdditionalFontList[c][0],
					this.aAdditionalFontList[c][1],
					0,
					'',
					'',
					1,
	...
	_onMouseupCover: function (d) {
		...
		a.fireEvent('mouseup', { left: f.left, middle: f.middle, right: f.right });
	},
	...
	_addFontToMenu: function (c, b, d) {
		...
		a.innerHTML = this.elFontItemTemplate.innerHTML
			.replace('@DisplayName@', c)
			.replace('FontFamily', b)
			.replace('@SampleText@', d);
	...
	$ON_EVENT_FONTNAME_LAYER_CLICKED: function (b) {
		...
		var d =
			this.htAllFonts[
				c.replace(/\"/g, nhn.husky.SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS)
			];
		var a;
		if (d) {
			a = d.defaultSize + 'pt';
	...
	$ON_SET_FONTFAMILY: function (c, b) {
		...
		if (parseInt(b, 10) > 0) {
			this.oApp.exec('SET_WYSIWYG_STYLE', [{ fontSize: b }]);
		}
		this.oApp.exec('SET_WYSIWYG_STYLE', [{ fontFamily: c }]);
		this.bDoNotRecordUndo = false;
	...
	_getMatchingLI: function (e) {
		...
		for (var b = 0; b < this.aLIFontNames.length; b++) {
			...
			for (var d = 0; d < a.length; d++) {
				if (
					!!a[d] &&
					jindo.$S(a[d].replace(/['"]/gi, '')).trim().$value() == e
				) {
					return c;
				}
			}
...
nhn.husky.SE2M_FontSizeWithLayerUI = jindo.$Class({
	...
	$ON_MSG_APP_READY: function () {
		...
		for (var a = 0; a < this.aLIFontSizes.length; a++) {
			this.oApp.registerBrowserEvent(
				...
				'click',
				'SET_FONTSIZE',
				[this._getFontSizeFromLI(this.aLIFontSizes[a])],
			);
		}
	},
	...
	$ON_SET_FONTSIZE: function (a) {
		...
		this.oApp.exec('SET_WYSIWYG_STYLE', [{ fontSize: a }]);
		this.oApp.exec('HIDE_ACTIVE_LAYER', []);
		this.oApp.exec('CHECK_STYLE_CHANGE', []);
	},
		...
		if (b.ie) {
			jindo.$A(this.oSelection.getNodes(true)).forEach(function (c, d, e) {
				if (
					...
					c.getAttribute('s_type').toLowerCase() == 'db'
				) {
					a = false;
					jindo.$A.Break();
				}
			...
			if (this.oSelection.collapsed) {
				...
				if (h == '') {
					...
					if (this._isExceptional()) {
						...
						this.oApp.exec('EXECCOMMAND', [
							'createLink',
							false,
							this.sATagMarker + b + encodeURIComponent(h),
							{ bDontAddUndoHistory: true },
						]);
					}
				}
				...
				for (var o = 0; o < k; o++) {
					...
					if (t && t.match(j)) {
						...
						if (r.ie) {
							jindo.$Element(s).attr({ href: d, target: sTarget });
						} else {
	...
	irToDb: function (k) {
		...
		for (var r = 0, m = d.length; r < m; r++) {
			...
			try {
				...
				u.innerHTML = (b + u.innerHTML).replace(
					/(&nbsp|\s)?(((?!http:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http:\/\/(?:(?!&nbsp;|\s|"|').)+))/gi,
	...
	$LOCAL_BEFORE_FIRST: function () {
		...
		for (var a = 0; a < this.aLIOptions.length; a++) {
			this.oApp.registerBrowserEvent(
				...
				[this._getLineHeightFromLI(this.aLIOptions[a])],
			);
		}
		this.oApp.registerBrowserEvent(
			this.oBtn_up,
...
nhn.husky.SE_WYSIWYGStyleGetter = jindo.$Class({
	...
	oStyleMap: {
		...
		lineHeight: {
			...
			converter: function (b, a) {
				...
				return (
					Math.ceil((parseInt(b, 10) / parseInt(a.fontSize, 10)) * 10) / 10
				);
			},
		},
		bold: { command: 'bold' },
	...
	$ON_MSG_APP_READY: function () {
		...
		if (
			...
			this.oStyleMap.textAlign = { type: 'Value', css: 'textAlign' };
		}
	},
	$ON_EVENT_EDITING_AREA_MOUSEUP: function (a) {
		this.oApp.exec('CHECK_STYLE_CHANGE');
	...
	_getStyle: function () {
		...
		for (attributeName in a) {
			if (this.oStyleMap[attributeName].converter) {
				a[attributeName] = this.oStyleMap[attributeName].converter(
					a[attributeName],
					a,
				);
			}
	...
	_wrapBlock: function (D, H) {
		...
		if (y.startContainer === y.endContainer) {
			if (y.startContainer.nodeType === 1 && y.startContainer.tagName === 'P') {
				if (
					nhn.husky.SE2M_Utils.isBlankNode(y.startContainer) ||
					nhn.husky.SE2M_Utils.isFirstChildOfNode(
						'IMG',
						y.startContainer.tagName,
						y.startContainer,
					) ||
					nhn.husky.SE2M_Utils.isFirstChildOfNode(
						'IFRAME',
...
nhn.husky.SE2M_TableCreator = jindo.$Class({
	...
	$ON_MSG_APP_READY: function () {
		this.sReEditGuideMsg_table = this.oApp.$MSG(
			nhn.husky.SE2M_Configuration.SE2M_ReEditAction.aReEditGuideMsg[3],
		);
		this.oApp.exec('REGISTER_UI_EVENT', [
			'table',
			'click',
	...
	irToDbDOM: function (e) {
		var f = [];
		var b = jindo.$$('table[class=__se_tbl]', e, { oneTimeOffCache: true });
		...
		for (var d = 0, c = f.length; d < c; d++) {
			a = f[d];
			jindo.$Element(a).css({ border: '', borderLeft: '', borderBottom: '' });
			jindo.$Element(a).attr({ border: 0, cellpadding: 1 });
			g = jindo.$$('tbody>tr>td', a);
			jindo.$A(g).forEach(function (k, j, h) {
				jindo.$Element(k).css({ border: '', borderTop: '', borderRight: '' });
			});
		}
	},
	dbToIrDOM: function (e) {
		...
		for (var d = 0, c = f.length; d < c; d++) {
			...
			jindo.$Element(a).attr({ border: 1, cellpadding: 0 });
			g = jindo.$$('tbody>tr>td', a);
			jindo.$A(g).forEach(function (k, j, h) {
				jindo
					.$Element(k)
	...
	_doGetTableString: function (d, k, h, j, n, l) {
		...
		var c = this.aTableStyleByBorder[l]
...
nhn.husky.SE2M_TableEditor = jindo.$Class({
				...
				.$Fn(function (c) {
					...
					for (var f = 0; f < a.length; f++) {
						...
						for (var d = 0, e = b.length; d < e; d++) {
							if (b[d].className) {
								b[d].className = b[d].className.replace(
									this.CELL_SELECTION_CLASS,
									'',
								);
								if (b[d].getAttribute(this.TMP_BGC_ATTR)) {
									b[d].style.backgroundColor = b[d].getAttribute(
										this.TMP_BGC_ATTR,
									);
									...
									if (b[d].getAttribute(this.TMP_BGIMG_ATTR)) {
										jindo
											.$Element(this.aCells[d])
											.css(
												'backgroundImage',
												b[d].getAttribute(this.TMP_BGIMG_ATTR),
											);
	...
	$LOCAL_BEFORE_FIRST: function (a) {
		...
		this.nPageLeftRightMargin =
			parseInt(b.css('marginLeft'), 10) + parseInt(b.css('marginRight'), 10);
	...
	$ON_TABLE_QE_SELECT_TEMPLATE: function (e) {
		...
		this.oApp.exec('RECORD_UNDO_ACTION', [
			'CHANGE_TABLE_STYLE',
			{ elSaveTarget: a, sSaveTarget: d, bDontSaveSelection: true },
		]);
	},
	...
	$ON_TABLE_QE_SET_IMG: function (e) {
		...
		if (a === '0') {
			...
			for (var d = 0, h = this.aSelectedCells.length; d < h; d++) {
				jindo
					.$Element(this.aSelectedCells[d])
					.css(
						'backgroundImage',
						'url(http://static.se2.naver.com/static/img/' + c + ')',
					);
		...
		switch (this.nStatus) {
			case this.STATUS.S_0:
				...
				if (b && b.tagName == 'TD') {
					...
					if (
						!jindo.$Element(a).hasClass(this._sSETblClass) &&
						!jindo.$Element(a).hasClass(this._sSEReviewTblClass)
					) {
						return;
					}
	...
	_coverResizeLayer: function () {
		...
		this.elResizeCover.style.height =
			a.height - this.nPageTopBottomMargin + 'px';
	...
	_reassignCellSizes: function (t) {
		var d = new Array(2);
		d[0] = jindo.$$('>TBODY>TR>TD', t, { oneTimeOffCache: true });
		d[1] = jindo.$$('>TBODY>TR>TH', t, { oneTimeOffCache: true });
		...
		for (var p = 0; p < 2; p++) {
			for (var s = 0; s < d[p].length; s++) {
				...
				if (g.getComputedStyle) {
					...
					o = this.parseBorder(
						b.css('borderLeftWidth'),
						b.css('borderLeftStyle'),
					);
					f = this.parseBorder(
						b.css('borderRightWidth'),
						b.css('borderRightStyle'),
					);
					e = this.parseBorder(
						b.css('borderTopWidth'),
						b.css('borderTopStyle'),
					);
					z = this.parseBorder(
						b.css('borderBottomWidth'),
						b.css('borderBottomStyle'),
					);
				}
	...
	_isOnBorder: function (e) {
		...
		if (nBottomBorderCriteria <= b) {
			this.htResizing.nBorderTopPos =
				this.htResizing.elCell.offsetHeight + d - 1;
		...
		if (nRightBorderCriteria <= d) {
			this.htResizing.nBorderLeftPos =
				this.htResizing.elCell.offsetWidth + d - 1;
	...
	_getAllAffectedCells: function (d, a, o, m) {
		...
		if (o == 1) {
			...
			g = { aCellsBefore: f, aCellsAfter: c, nMinBefore: b, nMinAfter: e };
		} else {
			for (var l = 0; l < p; l++) {
	...
	_insertRowBelow: function () {
		...
		var h = jindo.$$('>TR', m, { oneTimeOffCache: true });
	...
	_getCellMapping: function (t) {
		var m = jindo.$$('>TBODY>TR', t, { oneTimeOffCache: true });
		var v = 0;
	...
	_getBasisCellPosition: function (b) {
		...
		for (a = 0; a < this.htMap.length; a++) {
			for (c = 0; c < this.htMap[a].length; c++) {
				if (this.htMap[a][c] == b) {
					return { x: a, y: c, elCell: b };
				}
			}
		}
		return { x: 0, y: 0, elCell: b };
	},
	...
	_clearAllTableStyles: function (b) {
		...
		b.style.backgroundColor = '';
		b.style.color = '';
		var d = jindo.$$('>TBODY>TR>TD', b, { oneTimeOffCache: true });
	...
	_hideTableTemplate: function (a) {
		if (a.getAttribute(this.ATTR_TBL_TEMPLATE)) {
			this._doApplyTableTemplate(
				a,
				nhn.husky.SE2M_TableTemplate[
					this.parseIntOr0(a.getAttribute(this.ATTR_TBL_TEMPLATE))
				],
				true,
			);
		}
	...
	_doApplyTableTemplate: function (s, a, j) {
		...
		var m = jindo.$$('>TBODY>TR', s, { oneTimeOffCache: true });
		var e = 0;
		if (o) {
			var e = 1;
...
nhn.husky.SE2M_QuickEditor_Common = jindo.$Class({
	...
	$ON_MSG_APP_READY: function () {
		...
		if (a) {
			this._bUseConfig =
				!!a.common && typeof a.common.bUseConfig !== 'undefined'
					? a.common.bUseConfig
					: true;
		}
		if (!this._bUseConfig) {
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN: function (a) {
		...
		if (d.keyCode == 8 || d.keyCode == 46) {
			...
			if (b.ie && b.nativeVersion > 8) {
				var c = jindo.$$.getSingle('DIV.husky_seditor_editing_area_container')
					.childNodes[0];
	...
	setData: function (sResult) {
		...
		this._environmentData = {
			...
			img: { isOpen: false, type: oResult.img, isFixed: false },
			review: {
				isOpen: false,
				type: oResult.review,
				isFixed: false,
	...
	setOpenType: function (b, a) {
		...
		if (
			typeof this._environmentData[b] == 'undefined' ||
			this._environmentData[b] == null
		) {
			this._environmentData[b] = {};
		}
		if (
			typeof this._environmentData[b].isOpen == 'undefined' ||
	...
	toggle: function (b, a) {
		...
		jindo
			.$Ajax(this._sAddTextAjaxUrl, { type: 'jsonp', onload: function () {} })
			.request({
	...
	positionCopy: function (b, a, c) {
		jindo
			.$Element(jindo.$$.getSingle('._' + c, this.currentEle))
			.css({ top: a, left: b });
	},
	...
	getPositionInfo: function (b, a) {
		...
		var e = { w: jindo.$Element(a).width(), h: jindo.$Element(a).height() };
	...
	getElementPosition: function (e, b) {
		...
		return { x: d.left - h + g, y: d.top - f + c };
	},
...
Shortcut.Helper = {
	keyInterpretor: function (d) {
		...
		if (b) {
			var d =
				Shortcut.Store.anthorKeyHash[b.toUpperCase()] ||
...
nhn.DraggableLayer = jindo.$Class({
	$init: function (c, b) {
		...
		var e =
			this.toInt(jindo.$Element(c).offset().left) - this.aBasePosition.left;
		var a = this._correctXY({ x: e, y: d });
		c.style.top = a.y + 'px';
		c.style.left = a.x + 'px';
		this.$FnMouseDown = jindo.$Fn(
			jindo.$Fn(this._mousedown, this).bind(c),
			this,
		);
		this.$FnMouseMove = jindo.$Fn(
			jindo.$Fn(this._mousemove, this).bind(c),
			this,
		);
	...
	_mousedown: function (b, a) {
		...
		this.MouseOffsetY =
			a.pos().clientY - this.toInt(b.style.top) - this.aBasePosition.top;
		this.MouseOffsetX =
			a.pos().clientX - this.toInt(b.style.left) - this.aBasePosition.left;
...
nhn.husky.SE2M_ImgSizeRatioKeeper = jindo.$Class({
	...
	_isAdjustPossible: function (e) {
		var b = true;
		var a = this.oApp.htOptions.SE2M_EditingAreaRuler
			? this.oApp.htOptions.SE2M_EditingAreaRuler.bUse
			: false;
		if (a) {
			...
			if (d) {
				...
				if (e > c) {
					...
					alert(
						this.oApp
							.$MSG('SE2M_ImgSizeRatioKeeper.exceedMaxWidth')
...
nhn.husky.SE2M_Utils = {
	...
	getUniqueId: function (a) {
		return (
			(a || '') + jindo.$Date().time() + (Math.random() * 100000).toFixed()
		);
	},
	clone: function (c, b) {
		if (
	...
	iframeAlignConverter: function (f, b) {
		...
		if (e == 'DIV' || e == 'P') {
			...
			var h = d.html();
			var c =
				jindo.$Element(f).attr('align') || jindo.$Element(f).css('text-align');
...
(function () {
	...
	IsInstalledFont = function (g) {
		...
		a.style.cssText =
			h + 'font-family:"' + g.replace(b, '","') + '", "' + c + '" !important';
	...
	$init: function () {
		...
		if (!jindo.$Agent().navigator().ie) {
			...
			for (var b = 0, c = this.aInstantLoadTrigger.length; b < c; b++) {
				this['$BEFORE_' + this.aInstantLoadTrigger[b]] = jindo
					.$Fn(function () {
						this.loadSE2CSS();
					}, this)
					.bind();
			...
			for (var b = 0, c = this.aDelayedLoadTrigger.length; b < c; b++) {
				...
				this['$BEFORE_' + this.aDelayedLoadTrigger[b]] = jindo
					.$Fn(function (d) {
...
nhn.husky.SE2B_Customize_ToolBar = jindo.$Class({
	...
	_assignHTMLElements: function (a) {
		...
		this.elTextMoreButton = jindo.$$.getSingle(
			'button.se2_text_tool_more',
