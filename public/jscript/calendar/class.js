...
var calendar = {
	...
	replaceAll: function (value, oldChar, newChar) {
		if (value == null || value.replace(/ /g, '') == '') return;
			...
			if (this.isMSIE) return value.replace(new RegExp(oldChar, 'gi'), newChar);
			else return value.split(oldChar).join(newChar);
		}
	},
	...
	inputDate: function (year, month, day) {
		...
		if (this.mode == 'SEL') {
			if (
				!this.isDate(this.targetYear, realYear) ||
				!this.isDate(this.targetMonth, realMonth) ||
				!this.isDate(this.targetDay, realDay)
			) {
	...
	makeContent: function (year, month, day) {
		...
		var tag =
			...
			'<img src="' +
			this.path +
			'/images/btn_left.gif" onClick="parent.calendar.show(' +
			(year - 1) +
			',' +
			month +
			',' +
			...
			'<img src="' +
			this.path +
			'/images/btn_right.gif" onClick="parent.calendar.show(' +
			(year + 1) +
			',' +
			month +
			',' +
			...
			')" />' +
			'</div>' +
			'<div class="b"><span onClick="parent.calendar.show(' +
			this.now.year +
			',' +
			this.now.month +
			',' +
			...
			')">Today</span></div>' +
			'</div>' +
			'<table class="frame" onMouseOver="parent.calendar.hover(event, true)" onMouseOut="parent.calendar.hover(event, false)" onClick="parent.calendar.pickDate(event,' +
			year +
			',' +
			month +
			')">' +
		...
		if (column > 0 && column < 7) {
			for (i = 1, len = 7 - column; i <= len; i++) tag += '<td></td>';
		}
	...
	setDate: function () {
		...
		setTimeout(function () {
			calendar.registEvent('click', calendar.hide);
		}, 100);
	...
	show: function (year, month, day) {
		...
		setTimeout(function () {
			...
			objDoc.write(
				'body { width:100%; height:100%; background-color:#FFF; margin:0; overflow:hidden; }',
			);
			objDoc.write('div { font-family:tahoma; font-size:12px; }');
			objDoc.write('table { border-collapse:collapse; table-layout:fixed; }');
			objDoc.write(
				'th, td { padding:0; font-family:tahoma; font-size:12px; text-align:center; }',
			);
			objDoc.write(
				'.wrap { background-color:#EFE7E7; padding:2px; border:solid 1px #CCCCCC; overflow:hidden; }',
			);
			objDoc.write(
				'	.canvas { height:' +
					calendar.height +
					'px; background-color:#FFF; overflow:hidden; }',
			);
			...
			objDoc.write(
				'		.head .d div.blank { width:3px; font-size:1px; line-height:1px; }',
			);
			objDoc.write(
				'		.head .d img { display:block; float:left; cursor:pointer; margin-top:2px; }',
			);
			objDoc.write('		.head .b { float:right; margin:5px 5px 0 0; }');
			objDoc.write(
				'		.head .b span { font-size:10px; font-weight:bold; color:#CC0000; cursor:pointer }',
			);
			objDoc.write(
				'		.frame { border-collapse:collapse; width:' +
					calendar.width +
					'px; table-layout:fixed; }',
			);
			objDoc.write(
				'		.frame th { height:20px; color:#000; background-color:#F4F4F4; }',
			);
			objDoc.write('		.frame th.red { color:#FF0000; }');
			objDoc.write('		.frame th.blue { color:#0000FF; }');
			objDoc.write('		.frame td { height:18px; }');
			objDoc.write(
				'		.frame td.today { color:#CE0000; font-weight:bold; border:1px solid #FFCACA; }',
			);
			...
			objDoc.write('</head>');
			objDoc.write(
				'<body>' + calendar.makeContent(year, month, day) + '</body>',
			);
			objDoc.write('</html>');
			objDoc.close();
	...
	open: function (e, element, dateType) {
		...
		this.mode = '';
		this.target =
			element.nodeType == 1 ? element : document.getElementById(element);
		...
		if (element.nodeType == 1) {
			...
			this.posX = this.isMSIE
				? parseInt(document.body.scrollLeft, 10) + parseInt(e.clientX, 10)
				: parseInt(e.pageX, 10);
			this.posY = this.isMSIE
				? parseInt(e.clientY, 10) + parseInt(document.body.scrollTop, 10)
				: parseInt(e.pageY, 10);
	...
	openSelect: function (e, elementYear, elementMonth, elementDay, dateType) {
		...
		this.mode = 'SEL';
		this.targetYear =
			elementYear.nodeType == 1
				? elementYear
				: document.getElementById(elementYear);
		this.targetMonth =
			elementMonth.nodeType == 1
				? elementMonth
				: document.getElementById(elementMonth);
		this.targetDay =
			elementDay.nodeType == 1
				? elementDay
				: document.getElementById(elementDay);
		this.dateType = dateType;
