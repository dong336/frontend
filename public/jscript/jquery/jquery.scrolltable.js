...
(function ($) {
	$.widget('ui.scrolltable', {
		...
		destroy: function () {
			...
			$self
				.removeClass('st-container')
				.append($self.find('.st-head-table>colgroup'))
				.find('>thead')
				.replaceWith($self.find('.st-head-table>thead'))
				.end()
				.find('>tbody')
				.replaceWith($self.find('.st-body-table>tbody'))
				.end()
				.find('.st-head')
				.closest('tr')
				.remove()
			...
			$self
				...
				.add($self.find('>tbody>tr').eq(0).find('td'));
			/*	Remove - By landboy
			.width(function(){
		...
		var $body = $('<table class="st-body-table" cellpadding="0" cellspacing="0" border="0" />').append($container.find('>tbody')).css('width', '100%');*/
			var $head = $(
				'<table class="st-head-table" cellpadding="0" cellspacing="0" border="0" />',
			)
				.append($container.find('>colgroup').clone())
				.append($container.find('>thead'))
				.css('width', '100%');
			var $body = $(
				'<table class="st-body-table" cellpadding="0" cellspacing="0" border="0" />',
			)
				.append($container.find('>colgroup').clone())
				.append($container.find('>tbody'))
				.css('width', '100%');
			...
			$container
				...
				.html(
					'<thead><tr><td class="st-head"></td></tr></thead><tbody><tr><td class="st-body"><div class="st-body-scroll"></div></td></tr></tbody>',
				)
				.find('.st-head')
				.css('padding', '0 17px 0 0')
				.append($head)
				.end()
				.find('.st-body')
				.css('padding', '0')
		...
		_stripe: function ($container) {
			var opts = this.options;
			$container
				.find('.st-body-scroll>table>tbody>tr')
				.filter(':odd')
				.addClass(opts.oddClass)
				.end()
				.filter(':even')
				.addClass(opts.evenClass);
		},
