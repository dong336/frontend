...
(function ($) {
		...
		open: function (opts, obj) {
			if (!$box.active) {
				...
				switch (types[0]) {
					...
					case 'swf':
						var html =
							'<object id="laybox-swf" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="100%" height="100%">' +
							...
							'<embed src="' +
							$box.opts.source +
							'" type="application/x-shockwave-flash" quality="high" width="100%" height="100%" wmode="transparent" allowScriptAccess="always" />' +
							'</object>';
	...
	$box.create = {
		mask: function () {
			...
			var $div = $('<div id="' + $box.ids.mask + '"></div>')
				.css({
					position: 'fixed',
					top: 0,
					left: 0,
		...
		loading: function () {
			...
			if ($box.opts.loadingImage) {
				var $div = $('<div id="' + $box.ids.loading + '"></div>')
					.css({
					...
					.append('<img src="' + $box.opts.loadingImage + '" />')
					.appendTo('body');
		...
		canvas: function () {
			...
			if ($box.opts.borderSize) {
				$div.css({
					border: $box.opts.borderSize + 'px solid ' + $box.opts.borderColor,
				});
			}
		...
		close: function () {
			...
			if ($canvas.length && $box.opts.closeImage) {
				$('<a id="' + $box.ids.close + '" href="#" />')
					...
					.append('<img src="' + $box.opts.closeImage + '" />')
					.appendTo($canvas.children());
			}
		},
	};
	...
	$box.resize = {
		...
		canvas: function () {
			...
			var vps = getViewPortSize(
				($box.viewPortPadding * 2 + getScrollbarSize()) * -1,
			);
	...
	$box.position = {
		...
		centered: {
			...
			canvas: function (direction) {
				...
				var width =
					$canvas.width() +
					($box.opts.borderSize + $box.opts.canvasPadding) * 2;
				var height =
					$canvas.height() +
					($box.opts.borderSize + $box.opts.canvasPadding) * 2;
				var left =
					scroll.left +
					($box.opts.positionLeft !== null
						? $box.opts.positionLeft
						: Math.round(($win.width() - width) / 2));
				var top =
					scroll.top +
					($box.opts.positionTop !== null
						? $box.opts.positionTop
