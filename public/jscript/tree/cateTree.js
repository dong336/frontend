	...
	hide: function (obj) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data != false) {
					var no = data.cate;
					var hidden = data.hidden.toString().toNumeric();
					...
					if (data != false) {
						...
						if (data.list) {
							for (var i=0, len=data.list.length; i<len; i++) {
								var no = data.list[i].cate;
								var code = cateTree.getCode(element.getAttribute('code'), no);
								var name = data.list[i].name.trim();
								var selected = (code == cateTree.selected.code || code == target ? 1 : 0);
								var child = (data.list[i].child.toString().toNumeric()>0 ? 1 : 0);
								var last = (i < (len - 1) ? 0 : 1);

								var hidden = 0;
								if (typeof (data.list[i].hidden) != 'undefined') {
									if (data.list[i].hidden.toString().toNumeric()) hidden |= cateTree.HIDDEN.ME;
