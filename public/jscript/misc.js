...
function checkInvalidPw(id, pw, min, max) {
	...
	if (!/^(?=.*[0-9])(?=.*[a-zA-Z])(?!.*[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]).*$/.test(pw))
		return 2;
...
function checkResNo(objResNo1, objResNo2) {
	...
	if (objResNo1.value.stripspace() == '' || objResNo2.value.stripspace() == '')
		return false;
...
function checkComNo(objComNo1, objComNo2) {
	...
	if (objComNo1.value.stripspace() == '' || objComNo2.value.stripspace() == '')
		return false;
...
function checkBizNo(objBizNo1, objBizNo2, objBizNo3) {
	...
	else bizNo = objBizNo1.value + objBizNo2.value + objBizNo3.value; // 사업자번호 10자리
...
// 로그인 ##################################################
function gotoLogin() {
	//location.href = "../member/login?redirect="+escape(document.URL);
